...
def create_mock_pdb(path, chain_id="A", res_start=1, res_end=3, atom_start=1, ligand=False, ligand_chain_id=None):
    ...
    for res in range(res_start, res_end + 1):
        for atom, x, y, z in [
            ...
            pdb_lines.append(
                "ATOM  {serial:5d} {atom:^4} ALA {chain}{res:4d}    {x:8.3f}{y:8.3f}{z:8.3f}  1.00 20.00           {element:>2}"
                .format(
                    serial=atom_serial,
                    atom=atom,
                    chain=chain_id,
    ...
    if ligand:
        ...
        for i, (atom, x, y, z) in enumerate([
            ...
            pdb_lines.append(
                "HETATM{serial:5d} {atom:^4} LIG {chain}{res:4d}    {x:8.3f}{y:8.3f}{z:8.3f}  1.00 30.00           {element:>2}"
                .format(
                    serial=atom_serial,
                    atom=atom,
                    chain=lig_cid,
...
def test_combine_pdbs():
   ...
   # Parse combined.pdb and check chain IDs
    ...
    assert len(chains_in_pdb) == 2, f"Expected 2 chains, found {len(chains_in_pdb)}: {chains_in_pdb}"

    # Check that the chains in the file match the new chain IDs in chain_map
    mapped_chains = set()
    for mapping in chain_map.values():
        mapped_chains.update(mapping.values())
    assert set(chains_in_pdb) == mapped_chains, f"Chains in file ({chains_in_pdb}) do not match map ({mapped_chains})"
    ...
    assert len(chain_ids) == 4, f"Expected at least 2 chains, found {len(chain_ids)}: {chain_ids}"
    ...
    all_mapped_chains = {v for mapping in chain_map_ligand.values() for v in mapping.values()}
