...
def test_run_msd_single_constraint():
    cmd = [
        ...
        "run.py",
        "--out_folder", "./test/integration/outputs/single_constraint",
...
def test_run_msd_mulitple_constraint():
    cmd = [
        sys.executable, # this runs the current Python interpreter
        "run.py",
        "--out_folder", "./test/integration/outputs/msd_multiple_constraint",
        "--multi_state_pdb_path", "./test/integration/msd_multi_constraint_pdbs.json",
    ...
    # A1–A128 check
    assert seq_1a0o[0][:128] == seq_chainA_monomer[0][:128], \
        "Mismatch in A1–A128 between 1a0o and 1a0o_chainA_monomer"

    # B1–B70 check
    assert seq_1a0o[1][:70] == seq_chainB_monomer[0][:70], \
